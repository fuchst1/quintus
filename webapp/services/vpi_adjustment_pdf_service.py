...
class VpiAdjustmentPdfGenerationError(RuntimeError):
    """Eindeutiger Fehler für fehlgeschlagene VPI-PDF-Erstellung."""


class VpiAdjustmentPdfService:
    """Erzeugt VPI-Wertsicherungsbriefe primär über WeasyPrint (HTML -> PDF)."""
    ...
    def weasyprint_available() -> bool:
        if VpiAdjustmentPdfService._WEASYPRINT_RUNTIME_OK is not None:
            return VpiAdjustmentPdfService._WEASYPRINT_RUNTIME_OK

        try:
            VpiAdjustmentPdfService._patch_pydyf_compat()
            from weasyprint import HTML  # noqa: F401

            HTML(string="<html><body>ok</body></html>").write_pdf()
        except Exception:
            VpiAdjustmentPdfService._WEASYPRINT_RUNTIME_OK = False
            return False
        VpiAdjustmentPdfService._WEASYPRINT_RUNTIME_OK = True
        return True

    @classmethod
    def _render_html(cls, *, payload: dict[str, object]) -> str:
        return render_to_string(
            "webapp/letters/vpi_letter_pdf.html",
            {
                "payload": payload,
    ...
    def generate_letter_pdf(cls, *, payload: dict[str, object]) -> bytes:
        ...
        try:
            ...
            raise VpiAdjustmentPdfGenerationError(
        ...
        try:
            ...
            cls._WEASYPRINT_RUNTIME_OK = False
            cls.logger.exception("WeasyPrint-PDF-Erzeugung fehlgeschlagen (VPI).")
            raise VpiAdjustmentPdfGenerationError(
